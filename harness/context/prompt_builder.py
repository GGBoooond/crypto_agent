...
class PromptBuilder:
    ...
    def _parse_skill(self, path: Path) -> Optional[Dict[str, Any]]:
        ...
        regimes = self._extract_regime_tags(frontmatter)
        return {
    ...
    def _select_relevant_skills(self, regime: str) -> List[Dict[str, Any]]:
        if self.snapshot is None:
            return []
        matched: List[Dict[str, Any]] = []
        for skill in self.snapshot.skill_payloads:
            regimes = skill.get("regimes") or []
            if not regimes or regime in regimes:
